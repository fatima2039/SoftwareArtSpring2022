...
void ofApp::setup(){
    ...
    angle = -PI/2;
    noise_angle = ofRandom(50);
    noise_xloc = ofRandom(10);
    noise_yloc = ofRandom(10);
...
void ofApp::update(){
    ...
    noise_radius = ofRandom(10);
    
    //change radius using perlin noise
    noise_radius += 0.001;
    radius = (ofNoise(noise_radius)*400) + 1;
    ...
    noise_yloc += 0.01;
    noise_xloc += 0.01;
    float centerx = centerX + (ofNoise(noise_xloc) * 100) - 50;
    float centery = centerY + (ofNoise(noise_yloc) * 100) - 50;
    float radius1 = ofDegToRad(angle);
    float x1 = centerx + (radius * cos(radius1));
    float y1 = centery + (radius * sin(radius1));
    xloc1.push_back(x1);
    ...
    //color
    opacity.push_back(ofRandom(0,60));
...
void ofApp::draw(){
    ...
    ofSetLineWidth(4);
    ofSetColor(24, 255, 0);
    ofDrawCurve(centerX + 100, centerY+ 100, centerX, centerY, centerX + ofNoise(centerX), ofGetHeight(), centerX + 200, centerY + 200);
    
    for (int i=0; i<xloc1.size(); i++) {
        ofSetLineWidth(0.5);
        ofSetColor(155,10,32,opacity[i]);
        ofDrawLine(ofNoise(xloc1[i])+xloc1[i], ofNoise(yloc1[i])+ yloc1[i], ofNoise(xloc2[i])+xloc2[i], ofNoise(yloc2[i])+yloc2[i]);
    }
